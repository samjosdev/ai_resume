The final answer is a single, comprehensive Gradio application that integrates all the specified features. This application provides a user interface for managing a fantasy cricket league, including creating users, players, matches, fantasy teams, and leagues. It uses a local SQLite database for persistence.

The following Python code is the complete, self-contained, and runnable Gradio demo. To run it, save the code as a Python file (e.g., `fantasy_cricket_demo.py`) and execute it from your terminal (`python fantasy_cricket_demo.py`). Ensure you have the necessary libraries installed: `gradio`, `pandas`, and `sqlalchemy`.
...
class FantasyTeam(Base):
    ...
    gameweek_data: Maptam(back_populates="fantasy_team", cascade="all, delete-orphan")
...
def get_choices(model, name_col, id_col="id"):
    """Generic function to get choices for a dropdown from the database."""
    try:
...
def handle_add_player_to_squad(team_id, player_id):
    ...
    try:
        with SessionLocal() as session:
            ...
            if float(team.bank_balance) < float(player.price): return f"Error: Insufficient funds. Bank: {team.bank_balance}, Price: {player.price}", gr.update(), gr.update()
...
def create_gradio_ui():
    with gr.Blocks(theme=gr.themes.Soft(), title="Fantasy Cricket UI") as demo:
        ...
        def update_all_dropdowns():
            return {
                ft_user_select: gr.update(choices=get_user_choices()), league_creator: gr.update(choices=get_user_choices()),
                stats_player_select: gr.update(choices=get_player_choices()), squad_player_select: gr.update(choices=get_player_choices()),
                stats_match_select: gr.update(choices=get_match_choices()),
                view_ft_select: gr.update(choices=get_fantasy_team_choices()), join_league_team: gr.update(choices=get_fantasy_team_choices()),
                join_league_select: gr.update(choices=get_league_choices()), view_league_select: gr.update(choices=get_league_choices()),
            }
        demo.load(update_all_dropdowns, [], [ft_user_select, league_creator, stats_player_select, squad_player_select, stats_match_select, view_ft_select, join_league_team, join_league_select, view_league_select])
