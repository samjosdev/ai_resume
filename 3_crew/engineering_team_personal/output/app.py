...
engine = create_engine(DATABASE_URL, connect_args={"check_same_thread": False}) # check_same_thread for SQLite in web apps

# This is required for SQLite to enforce foreign key constraints
...
def get_choices(model, name_col, id_col="id"):
    """Generic function to get choices for a dropdown."""
    with SessionLocal() as session:
        items = session.query(model).order_by(name_col).all()
...
def get_match_choices():
    with SessionLocal() as session:
        ...
        return [
            (f"GW{m.gameweek}: {m.home_team_name} vs {m.away_team_name}", m.id)
            for m in matches
        ]
...
def handle_get_users():
    with SessionLocal() as session:
        ...
        data = [{
            "Display Name": u.display_name, "Email": u.email,
            "Role": u.role.value, "Experience": u.experience_level.value if u.experience_level else "N/A",
            "Created": u.created_at.strftime('%Y-%m-%d'), "ID": u.id
        } for u in users]
        return pd.DataFrame(data)

# Player Management
def handle_create_player(full_name, real_team, role, price, injury_status):
    if not all([full_name, real_team, role, price, injury_status]):
        return "Error: All fields are required.", gr.update(), gr.update(value=str(uuid.uuid4()))
    try:
        with SessionLocal() as session:
            new_player = Player(
                full_name=full_name, real_team_name=real_team,
                role=PlayerRole[role], price=float(price),
                injury_status=PlayerInjuryStatus[injury_status]
            )
...
def handle_get_players():
    with SessionLocal() as session:
        ...
        data = [{
            "Name": p.full_name, "Team": p.real_team_name, "Role": p.role.value,
            "Price": f"{p.price:.2f}", "Status": p.injury_status.value, "ID": p.id
        } for p in players]
        return pd.DataFrame(data)
...
def handle_create_match(gameweek, home_team, away_team, match_date, status):
    ...
    try:
        with SessionLocal() as session:
            new_match = Match(
                gameweek=int(gameweek), home_team_name=home_team,
                away_team_name=away_team, match_date=match_date,
...
def handle_get_matches():
    with SessionLocal() as session:
        ...
        data = [{
            "Gameweek": m.gameweek, "Home": m.home_team_name, "Away": m.away_team_name,
            "Date": m.match_date.strftime('%Y-%m-%d %H:%M'), "Status": m.status.value, "ID": m.id
        } for m in matches]
        return pd.DataFrame(data)
        ...
        with SessionLocal() as session:
            ...
            if existing_stats:
                ...
                for key, value in stats_dict.items():
                    setattr(existing_stats, key, value or 0)
...
def handle_get_match_stats(match_id):
    ...
    with SessionLocal() as session:
        ...
        data = [{
            "Player": s.player.full_name, "Runs": s.runs_scored, "Wickets": s.wickets_taken,
            "Catches": s.catches, "Points": s.points
        } for s in stats]
        return pd.DataFrame(data)
...
def handle_get_fantasy_teams():
    with SessionLocal() as session:
        ...
        data = [{
            "Team Name": t.team_name, "Owner": t.user.display_name,
            "Bank": f"{t.bank_balance:.2f}", "Created": t.created_at.strftime('%Y-%m-%d'), "ID": t.id
        } for t in teams]
        return pd.DataFrame(data)
...
def handle_get_team_details(team_id):
    ...
    with SessionLocal() as session:
        ...
        owner = team.user.display_name
        balance = f"{team.bank_balance:.2f}"
        ...
        # Simple points data for demo
        points_data = [{"Gameweek": gw, "Points": random.randint(30, 80)} for gw in range(1, 5)]
    ...
    try:
        with SessionLocal() as session:
            ...
            if not team or not player:
                return "Error: Invalid team or player.", gr.update(), gr.update()
            
            if float(team.bank_balance) < float(player.price):
                return f"Error: Insufficient funds. Bank: {team.bank_balance}, Price: {player.price}", gr.update(), gr.update()

            if session.query(FantasySquadPlayer).filter_by(fantasy_team_id=team_id, player_id=player_id).first():
                return "Error: Player is already in the squad.", gr.update(), gr.update()
            ...
            _, _, new_squad_df, _ = handle_get_team_details(team_id)
            return f"Player '{player.full_name}' added to squad!", gr.update(value=f"{team.bank_balance:.2f}"), new_squad_df
    except Exception as e:
...
def handle_create_league(name, type, creator_id, start_gw):
    ...
    try:
        with SessionLocal() as session:
            invite_code = None
            if LeagueType[type] == LeagueType.PRIVATE:
                invite_code = ''.join(random.choices(string.ascii_uppercase + string.digits, k=8))
            
            new_league = League(
                name=name, type=LeagueType[type], creator_user_id=creator_id,
                start_gameweek=int(start_gw), invite_code=invite_code
            )
            session.add(new_league)
            session.commit()
            
            # Automatically add creator's team to the league if they have one
...
def handle_get_leagues():
    with SessionLocal() as session:
        ...
        data = [{
            "Name": l.name, "Type": l.type.value, "Creator": l.creator.display_name,
            "Start GW": l.start_gameweek, "Code": l.invite_code or "N/A", "ID": l.id
        } for l in leagues]
        return pd.DataFrame(data)
...
def handle_join_league(team_id, league_id, invite_code):
    ...
    try:
        with SessionLocal() as session:
            ...
            if league.type == LeagueType.PRIVATE and league.invite_code != invite_code:
                return "Error: Invalid invite code.", gr.update()

            if session.query(LeagueMembership).filter_by(league_id=league_id, fantasy_team_id=team_id).first():
                return "Error: Team is already in this league.", gr.update()
...
def handle_get_league_members(league_id):
    ...
    with SessionLocal() as session:
        ...
        data = [{
            "Rank": i + 1, "Team": m.fantasy_team.team_name,
            "Owner": m.fantasy_team.user.display_name, "Points": m.total_points
...
def create_gradio_ui():
    with gr.Blocks(theme=gr.themes.Soft(), title="Fantasy Cricket UI") as demo:
        ...
        # Hidden components to trigger updates across tabs
        trigger_user_update = gr.Textbox(label="trigger_user", visible=False)
        trigger_player_update = gr.Textbox(label="trigger_player", visible=False)
        trigger_match_update = gr.Textbox(label="trigger_match", visible=False)
        trigger_team_update = gr.Textbox(label="trigger_team", visible=False)
        trigger_league_update = gr.Textbox(label="trigger_league", visible=False)
            ...
            with gr.TabItem("Admin: Users & Players"):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        ...
                        users_df = gr.DataFrame(value=handle_get_users, interactive=False)
                ...
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        ...
                        players_df = gr.DataFrame(value=handle_get_players, interactive=False)

            with gr.TabItem("Admin: Matches & Stats"):
                with gr.Row():
                    with gr.Column(scale=1):
                        gr.Markdown("## Create Match")
                        match_gw = gr.Number(label="Gameweek", precision=0)
                    ...
                    with gr.Column(scale=2):
                        ...
                        matches_df = gr.DataFrame(value=handle_get_matches, interactive=False)
                ...
                with gr.Row():
                    with gr.Column(scale=1):
                        stats_df_input = gr.DataFrame(
                            headers=["runs_scored", "balls_faced", "fours", "sixes", "wickets_taken", "overs_bowled", "runs_conceded", "maidens_bowled", "catches", "stumpings", "run_outs", "points"],
                            value=[[0]*12],
                            row_count=(1, "fixed"), col_count=(12, "fixed"),
                            interactive=True,
                        )
                        ...
                        with gr.Tabs():
                            with gr.TabItem("Squad"):
                                with gr.Row():
                                    ...
                                    with gr.Column():
                                        view_ft_squad_df = gr.DataFrame(label="Current Squad", interactive=False)
                            with gr.TabItem("Points History"):
                                view_ft_points_df = gr.DataFrame(label="Gameweek Points", interactive=False)
            ...
            with gr.TabItem("League Management"):
                with gr.Row():
                    ...
                    with gr.Column(scale=2):
                        ...
                        leagues_df = gr.DataFrame(value=handle_get_leagues, interactive=False)
                        ...
                        league_members_df = gr.DataFrame(interactive=False)

        # --- Event Listeners ---

        # Triggers for updating dropdowns globally
        trigger_user_update.change(fn=get_user_choices, outputs=[ft_user_select, league_creator])
        trigger_player_update.change(fn=get_player_choices, outputs=[stats_player_select, squad_player_select])
        trigger_match_update.change(fn=get_match_choices, outputs=[stats_match_select])
        trigger_team_update.change(fn=get_fantasy_team_choices, outputs=[view_ft_select, join_league_team])
        trigger_league_update.change(fn=get_league_choices, outputs=[join_league_select, view_league_select])
        
        # User & Player Admin Tab
